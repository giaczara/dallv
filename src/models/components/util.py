...
def load_state(network, state_dict):
    ...
    for i, _ in enumerate(net_state_keys_copy):
        ...
        if name_pretrained in state_dict.keys():
            ...
            if state_dict[name_pretrained].shape == dst_param_shape:
                network.state_dict()[name].copy_(
                    state_dict[name_pretrained].view(dst_param_shape)
                )
    ...
    if net_state_keys:
        ...
        pruned_additional_states = [
            x for x in net_state_keys if x not in num_batches_list
        ]

        if pruned_additional_states:
            logging.info(
                "There are layers in current network not initialized by pretrained"
            )
